...
from sqlglot import exp, generator, parser, tokens, transforms,transpile
...
def _add_date_sql(self: Hive2.Generator, expression: DATE_ADD_OR_SUB) -> str:
...
def _date_diff_sql(self: Hive2.Generator, expression: exp.DateDiff | exp.TsOrDsDiff) -> str:
    unit = expression.text("unit").upper()
...
def _json_format_sql(self: Hive2.Generator, expression: exp.JSONFormat) -> str:
...
def _array_sort_sql(self: Hive2.Generator, expression: exp.ArraySort) -> str:
    return self.func("SORT_ARRAY", expression.this)


def _str_to_unix_sql(self: Hive2.Generator, expression: exp.StrToUnix) -> str:
    return self.func("UNIX_TIMESTAMP", expression.this, time_format("hive")(self, expression))


def _unix_to_time_sql(self: Hive2.Generator, expression: exp.UnixToTime) -> str:
...
def _str_to_date_sql(self: Hive2.Generator, expression: exp.StrToDate) -> str:
    this = self.sql(expression, "this")
    time_format = self.format_time(expression)
    if time_format not in (Hive2.TIME_FORMAT, Hive2.DATE_FORMAT):
        this = f"FROM_UNIXTIME(UNIX_TIMESTAMP({this}, {time_format}))"
    return f"CAST({this} AS DATE)"


def _str_to_time_sql(self: Hive2.Generator, expression: exp.StrToTime) -> str:
    this = self.sql(expression, "this")
    time_format = self.format_time(expression)
    if time_format not in (Hive2.TIME_FORMAT, Hive2.DATE_FORMAT):
        this = f"FROM_UNIXTIME(UNIX_TIMESTAMP({this}, {time_format}))"
...
def _to_date_sql(self: Hive2.Generator, expression: exp.TsOrDsToDate) -> str:
    time_format = self.format_time(expression)
    if time_format and time_format not in (Hive2.TIME_FORMAT, Hive2.DATE_FORMAT):
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {
            **generator.Generator.TRANSFORMS,
            ...
            e: f"CAST(DATE_FORMAT({self.sql(e, 'this')}, {Hive2.DATEINT_FORMAT}) AS INT)",
            exp.DiToDate: lambda self,
            e: f"TO_DATE(CAST({self.sql(e, 'this')} AS STRING), {Hive2.DATEINT_FORMAT})",
...
def test_format_sql():
    ...
    sql =transpile(sql,read='hive', write=Hive2, identify=True, pretty=True)[0]
    print(sql)
